...
def parse_photos_and_colors(photo_raw, color_raw):
    ...
    colors_raw_list = (
        [x.strip() for x in c_str.split("/") if x.strip()] if c_str else []
    )
...
def get_products():
    try:
        ...
        for row in reader:
            ...
            photos, colors = parse_photos_and_colors(
                clean_row.get("Фото", ""), clean_row.get("Цвет", "")
            )
...
def faq_cmd(message):
    text = (
        ...
        "— Обирайте товари в каталозі, надилайте замовлення і ви зможете забрати його у магазині або узгодити доставку з менеджером.\n\n"
