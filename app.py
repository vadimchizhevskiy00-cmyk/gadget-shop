...
def parse_photos_and_colors(photo_raw, color_raw):
    p_str = clean_val(photo_raw)
    c_str = clean_val(color_raw)
    
    photos = [x.strip() for x in p_str.split(";") if x.strip()] if p_str else []
    colors_raw_list = [x.strip() for x in c_str.split("/") if x.strip()] if c_str else []
    
    colors = []
    for c in colors_raw_list:
        is_available = True
        name = c
        if c.endswith("(-)") or c.endswith("(нет)") or c.endswith("(немає)"):
            is_available = False
            name = c.replace("(-)", "").replace("(нет)", "").replace("(немає)", "").strip()
        colors.append({"name": name, "available": is_available})
...
def get_products():
    try:
        ...
        for row in reader:
            ...
            m_list, p_list = parse_memory_and_prices(clean_row.get('Память', ''), clean_row.get('Цена', ''))
            photos, colors = parse_photos_and_colors(clean_row.get('Фото', ''), clean_row.get('Цвет', ''))
            
            clean_row['memory_list'] = m_list
            clean_row['price_list'] = p_list
            clean_row['photo_list'] = photos
            clean_row['color_list'] = colors
...
def start_cmd(message):
    try:
        ...
        reply_kb.add(types.KeyboardButton(text="📱 Відкрити каталог", web_app=web_app))
        reply_kb.add(
...
def faq_cmd(message):
    text = (
        ...
        "2️⃣ <b>Як зробити замовлення?</b>\n"
        "— Обирайте товари в каталозі, надсилайте замовлення і ви зможете забрати його у магазині в зручний час.\n\n"
